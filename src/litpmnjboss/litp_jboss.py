...
from litpmnjboss import litp_jboss_app
...
class LitpJboss(object):
    ...
    def __init__(self, config):
        ...
        # These are filled in by self.set_config when we know the config
        # Basically this is to keep pylint happy
        self.jbosscli = None
        self._adduser = None
        self._mgmt_user_files = None
        self._jboss_console_log = None
        self._jboss_script = None
    ...
    def _check_vars(self, config):
        ...
        if config.get('home_dir') is None:
            common.log("Please specify JBoss home-dir", echo=True)
            self._exit(1)
        ...
        if config.get('management_listener') is None:
            common.log("Please specify management listener", echo=True)
            self._exit(1)
        ...
        if config.get('process_group') is None:
            common.log("Please specify process group", echo=True)
            self._exit(1)

        if config.get('management_user') is None:
            common.log("Please specify JBoss management user", echo=True)
            self._exit(1)

        if config.get('management_password') is None:
            common.log("Please specify JBoss management password", echo=True)
            self._exit(1)

    def _verify_pid(self):
        """
        Set self.pid if process is running.
    ...
    def _update_perms(self, path=None):
        ...
        rc, _, stderr = self._exec_cmd(cmd)
    ...
    def _start_all_apps(self):
        ...
        if len(self.config.apps) > 0:
            ...
            last_started = -1
            for idx, app_config in enumerate(self.config.apps):
    ...
    def _stop_all_apps_batch_worker(self, running_des):
        ...
        while True:
            ...
            common.log('About to stop %d deployable entities (%s) in a batch' \
                     % (len(running_des), ", ".join([app.get('name') for app in
                         running_des])))
            rv, stdout, _ = self.jbosscli.run_commands(batch, timeout)
            cli_timed_out = False
    ...
    def _start(self):
        """
        It starts the JBoss instance described by the environment
        ...
        # _check_startup_status
        if self._check_startup_status() != 0:
    ...
    def _stop(self, silent=False):
        """
        Will stop a started JBoss instance.
    ...
    def restart(self):
        """
        Restarts the JBoss instance by first stopping and then
        starting it.
    ...
    def reload(self):
        """
        Restarts the JBoss instance, same behaviour as restart.
        """
        return self.restart()
    ...
    def _get_ppid(self, pid):
        ...
        try:
            ...
            common.log("Failed: %s" % (e), level="DEBUG")
            return 1
        finally:
            f.close()
    ...
    def status(self, silent=False):
        """
        Will print the status of a given JBoss instance.
    ...
    def _untar_support_packages(self):
        ...
        for support_package in self.config.support_packages:
            cmd = 'tar -xzf %s -C %s' % \
    ...
    def _deploy(self):
        """
        Will deploy the JBoss instance described by the environment
    ...
    def _undeploy(self, remove_state_dir):
        ...
        msg = "Deleted JBoss \"%s\" and \"%s\"" % (self.config.get('home_dir'),
    ...
    def _redeploy(self):
        """
        It does a redeploy of JBoss which consist of
    ...
    def _upgrade(self):
        """
        It compares the installed version of jboss
...
if __name__ == "__main__":
    ...
    _config = litp_jboss_config.LitpJbossConfig(os.environ)
    LitpJboss(_config).main()
